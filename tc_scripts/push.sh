...
if ! pushd "${DIR}/sunrise_clang"; then
    git clone --single-branch -b main "https://${ghuser_name}:${GITHUB_TOKEN}@github.com/shelby-stuffs/sunrise_clang" --depth=1 ||
        {
...
export release_file="sunrise-clang-${short_clang}-${release_tag}.tar"
...
pushd "${DIR}/sunrise_clang" || exit 1
bash "${DIR}/tc_scripts/info.sh"
git add .
git commit -s -m "$(cat /tmp/commit_msg)"
git push "https://${ghuser_name}:${GITHUB_TOKEN}@github.com/shelby-stuffs/sunrise_clang" main -f
